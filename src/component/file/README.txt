...
This custom build includes both cleaning and building processes, so no separate action is required.

If there are no Korean characters detected, the script will immediately execute cleaning and building.

However, if Korean characters are found, the script will prompt the user with a warning and the option to proceed with cleaning and building.
...
해당 커스텀 빌드는 clean과 build를 포함하고 있어 별도로 실행할 필요가 없습니다.

만약 한글이 없는 경우, 즉시 clean과 build가 실행되며, 한글이 포함된 경우 경고 메시지와 함께 사용자가 clean과 build를 진행할지 여부를 선택할 수 있습니다.
